    ...
    <div className="pt-48 pb-32 px-4 relative bg-white">
        ...
        <div className="container mx-auto max-w-7xl relative z-10 overflow-hidden">
            <div className="lg:grid lg:grid-cols-12 lg:gap-16 items-start">
                ...
                <div className="lg:col-span-4 mb-16 lg:mb-0 lg:sticky lg:top-8 p-2 overflow-hidden">
                    <ScrollFadeIn
                    delay={200}
                    >
                        <div className="flex items-center mb-6">
                            <div className="h-[2px] w-12 mr-4 bg-[var(--site-black)]"></div>
                            <span className="text-4xl font-bold uppercase tracking-widest text-[var(--site-black)]">
                                Digital Strategy 
                            </span>
                        </div>
                        <h2 className="text-2xl font-bold mb-6 leading-tight text-black">
                             <span className='text-black'>About package</span>
                        </h2>
                        <p className="text-xl leading-relaxed text-black">
                            Audit & analysis of existing online presence social + web
                        </p>
                    </ScrollFadeIn>
                </div>
                    ...
                    {seoData.keypoints.map((point, index) => {
                        ...
                        return (
                            <ScrollFadeIn key={point.id} delay={index * 100} className="h-full overflow-hidden p-4">
                                <div 
                                     className="group p-8 rounded-2xl h-full border transition-all duration-500 hover:-translate-y-2 bg-[var(--site-purple)]"
                                >
                ...
                {seoData.packages.map((pkg, index) => {
                    ...
                    return (
                        <ScrollFadeIn key={pkg.id} delay={index * 150} className={`h-full ${isFeatured ? 'p-6 lg:mt-5 mt-4 md:-mt-8 mb-4 md:mb-8' : 'p-6 lg:mt-5 mt-4 md:-mt-8 mb-4 md:mb-8'}`}>
                            <div
                                className={`p-8 rounded-3xl border-none hover:translate-y-5 flex flex-col overflow-hidden relative transition-all duration-500 h-full border group hover:border-pink-500/50 bg-[var(--site-black)]`}
                            >
                                {isFeatured && (
                                ...
                                <ul className="space-y-4 mb-8 flex-grow">
                                    {seoData.keypoints.slice(0, index + 3).map((point, i) => ( 
                                        <li key={i} className="flex items-start">
                                            ...
                                            <span className="text-lg text-white">{point.title}</span>
                                        </li>
                                    ))}
                                </ul>

                                <button
                                    className={`hover:cursor-pointer hover:bg-[var(--site-purple)] w-full py-4 text-md font-bold uppercase rounded-xl transition-all duration-300 flex items-center justify-center text-white bg-[var(--site-pink)]`}
                                >
                                    Select Plan
                                </button>
                            </div>
